...
public class RegisterActivity extends AppCompatActivity {
    ...
    public void register(final View view) {
        ...
        if (TextUtils.isEmpty(email)){
            mEmail.setError("Email é um campo obrigatório.");
            return;
        }

        if (TextUtils.isEmpty(password)){
            mPassword.setError("Password é um campo obrigatório.");
            return;
        }

        if (password.length() < 6){
            mPassword.setError("Password deve ter, no mínimo, 6 caracteres");
            return;
        }
